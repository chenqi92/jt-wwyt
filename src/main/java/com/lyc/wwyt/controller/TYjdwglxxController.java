...
public class TYjdwglxxController {
     ...
     * 查询当前账户下所有应急队伍管理信息表     *
     *
     * @return List<TYjdwglxxEntity> 当前账户下所有应急队伍管理信息表
     */
    @Operation(description = "查询应急队伍管理信息表信息表数据", summary = "查询应急队伍管理信息表信息表数据", tags = {"查询所有数据"})
    @GetMapping
    @Idempotent(expireTime = 180, info = "3分钟内最多请求一次!")
    public List<TYjdwglxxVO> selectAll() {
        return this.tYjdwglxxService.selectList();
    }

    /**
     * 分页查询当前账户下所有应急队伍管理信息表     *
     *
     * @return List<TYjdwglxxDTO> 分页当前账户下所有应急队伍管理信息表
     */
    ...
    public IPage<TYjdwglxxVO> selectPage(@ParameterObject Page<TYjdwglxxDTO> page, @ParameterObject TYjdwglxxDTO tYjdwglxxDTO) {
        return this.tYjdwglxxService.selectPage(page, tYjdwglxxDTO);
    }
