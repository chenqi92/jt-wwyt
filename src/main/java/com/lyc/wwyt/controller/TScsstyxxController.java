...
public class TScsstyxxController {
     ...
     * 查询当前账户下所有生产设施停用信息信息
     *
     * @return List<TScsstyxxEntity> 当前账户下所有生产设施停用信息信息
     */
    @Operation(description = "查询生产设施停用信息信息表数据", summary = "查询生产设施停用信息信息表数据", tags = {"查询所有数据"})
    @GetMapping
    @Idempotent(expireTime = 180, info = "3分钟内最多请求一次!")
    @ExportExcel(name = "生产设施停用信息", sheets = @Sheet(sheetName = "t_scsstyxx"), headGenerator = CustomHead.class)
    public List<TScsstyxxVO> selectAll(@ParameterObject TScsstyxxDTO tScsstyxxDTO) {
        return this.tScsstyxxService.queryList(tScsstyxxDTO);
    }

    /**
     * 分页查询当前账户下所有生产设施停用信息信息
     *
     * @return List<TScsstyxxDTO> 分页当前账户下所有生产设施停用信息信息
     */
    ...
    public IPage<TScsstyxxVO> selectPage(@ParameterObject Page<TScsstyxxDTO> page, @ParameterObject TScsstyxxDTO tScsstyxxDTO) {
        return this.tScsstyxxService.queryPage(page, tScsstyxxDTO);
    }
    ...
    public List<TableInfoVO> heads() {
        // 查询所有列名
        List<TableInfoVO> queryInfos = this.commonService.queryTableHeaders("t_scsstyxx");
