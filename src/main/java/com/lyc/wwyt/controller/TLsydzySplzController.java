...
public class TLsydzySplzController {
    ...
    /**
     * 查询当前账户下所有临时用电作业审批流转记录表     *
     *
     * @return List<TLsydzySplzEntity> 当前账户下所有临时用电作业审批流转记录表
     */
    @Operation(description = "查询临时用电作业审批流转记录表信息表数据", summary = "查询临时用电作业审批流转记录表信息表数据", tags = {"查询所有数据"})
    @GetMapping
    @Idempotent(expireTime = 180, info = "3分钟内最多请求一次!")
    public List<TLsydzySplzVO> selectAll() {
        return this.tLsydzySplzService.selectList();
    }

    /**
     * 分页查询当前账户下所有临时用电作业审批流转记录表     *
     *
     * @return List<TLsydzySplzDTO> 分页当前账户下所有临时用电作业审批流转记录表
     */
    ...
    public IPage<TLsydzySplzVO> selectPage(@ParameterObject Page<TLsydzySplzDTO> page, @ParameterObject TLsydzySplzDTO tLsydzySplzDTO) {
        return this.tLsydzySplzService.selectPage(page, tLsydzySplzDTO);
    }
