...
public class TYjyaglxxController {
     ...
     * 查询当前账户下所有应急预案管理信息表     *
     *
     * @return List<TYjyaglxxEntity> 当前账户下所有应急预案管理信息表
     */
    @Operation(description = "查询应急预案管理信息表信息表数据", summary = "查询应急预案管理信息表信息表数据", tags = {"查询所有数据"})
    @GetMapping
    @Idempotent(expireTime = 180, info = "3分钟内最多请求一次!")
    public List<TYjyaglxxVO> selectAll() {
        return this.tYjyaglxxService.selectList();
    }

    /**
     * 分页查询当前账户下所有应急预案管理信息表     *
     *
     * @return List<TYjyaglxxDTO> 分页当前账户下所有应急预案管理信息表
     */
    ...
    public IPage<TYjyaglxxVO> selectPage(@ParameterObject Page<TYjyaglxxDTO> page, @ParameterObject TYjyaglxxDTO tYjyaglxxDTO) {
        return this.tYjyaglxxService.selectPage(page, tYjyaglxxDTO);
    }
