...
public class TAqfxsjController {
     ...
     * 查询当前账户下所有安全风险事件表     *
     *
     * @return List<TAqfxsjEntity> 当前账户下所有安全风险事件表
     */
    @Operation(description = "查询安全风险事件表信息表数据", summary = "查询安全风险事件表信息表数据", tags = {"查询所有数据"})
    @GetMapping
    @Idempotent(expireTime = 180, info = "3分钟内最多请求一次!")
    public List<TAqfxsjVO> selectAll() {
        return this.tAqfxsjService.selectList();
    }

    /**
     * 分页查询当前账户下所有安全风险事件表     *
     *
     * @return List<TAqfxsjDTO> 分页当前账户下所有安全风险事件表
     */
    ...
    public IPage<TAqfxsjVO> selectPage(@ParameterObject Page<TAqfxsjDTO> page, @ParameterObject TAqfxsjDTO tAqfxsjDTO) {
        return this.tAqfxsjService.selectPage(page, tAqfxsjDTO);
    }
