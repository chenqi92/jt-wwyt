...
public class TAqhdxxController {
     ...
     * 查询当前账户下所有安全活动信息表     *
     *
     * @return List<TAqhdxxEntity> 当前账户下所有安全活动信息表
     */
    @Operation(description = "查询安全活动信息表信息表数据", summary = "查询安全活动信息表信息表数据", tags = {"查询所有数据"})
    @GetMapping
    @Idempotent(expireTime = 180, info = "3分钟内最多请求一次!")
    public List<TAqhdxxVO> selectAll() {
        return this.tAqhdxxService.selectList();
    }

    /**
     * 分页查询当前账户下所有安全活动信息表     *
     *
     * @return List<TAqhdxxDTO> 分页当前账户下所有安全活动信息表
     */
    ...
    public IPage<TAqhdxxVO> selectPage(@ParameterObject Page<TAqhdxxDTO> page, @ParameterObject TAqhdxxDTO tAqhdxxDTO) {
        return this.tAqhdxxService.selectPage(page, tAqhdxxDTO);
    }
