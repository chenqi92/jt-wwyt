...
public class TScssbfxxController {
     ...
     * 查询当前账户下所有生产设备报废信息信息
     *
     * @return List<TScssbfxxEntity> 当前账户下所有生产设备报废信息信息
     */
    @Operation(description = "查询生产设备报废信息信息表数据", summary = "查询生产设备报废信息信息表数据", tags = {"查询所有数据"})
    @GetMapping
    @Idempotent(expireTime = 180, info = "3分钟内最多请求一次!")
    @ExportExcel(name = "生产设备报废信息", sheets = @Sheet(sheetName = "t_scssbfxx"), headGenerator = CustomHead.class)
    public List<TScssbfxxVO> selectAll(@ParameterObject TScssbfxxDTO tScssbfxxDTO) {
        return this.tScssbfxxService.queryList(tScssbfxxDTO);
    }

    /**
     * 分页查询当前账户下所有生产设备报废信息信息
     *
     * @return List<TScssbfxxDTO> 分页当前账户下所有生产设备报废信息信息
     */
    ...
    public IPage<TScssbfxxVO> selectPage(@ParameterObject Page<TScssbfxxDTO> page, @ParameterObject TScssbfxxDTO tScssbfxxDTO) {
        return this.tScssbfxxService.queryPage(page, tScssbfxxDTO);
    }
    ...
    public List<TableInfoVO> heads() {
        // 查询所有列名
        List<TableInfoVO> queryInfos = this.commonService.queryTableHeaders("t_scssbfxx");
