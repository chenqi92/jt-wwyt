...
public class TDwjcxxController {
     ...
     * 查询当前账户下所有点位检查信息表     *
     *
     * @return List<TDwjcxxEntity> 当前账户下所有点位检查信息表
     */
    @Operation(description = "查询点位检查信息表信息表数据", summary = "查询点位检查信息表信息表数据", tags = {"查询所有数据"})
    @GetMapping
    @Idempotent(expireTime = 180, info = "3分钟内最多请求一次!")
    public List<TDwjcxxVO> selectAll() {
        return this.tDwjcxxService.selectList();
    }

    /**
     * 分页查询当前账户下所有点位检查信息表     *
     *
     * @return List<TDwjcxxDTO> 分页当前账户下所有点位检查信息表
     */
    ...
    public IPage<TDwjcxxVO> selectPage(@ParameterObject Page<TDwjcxxDTO> page, @ParameterObject TDwjcxxDTO tDwjcxxDTO) {
        return this.tDwjcxxService.selectPage(page, tDwjcxxDTO);
    }
