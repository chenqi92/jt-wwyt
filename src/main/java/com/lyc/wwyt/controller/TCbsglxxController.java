...
public class TCbsglxxController {
    ...
    public List<TCbsglxxEntity> selectAll() {
     ...
     * 分页查询当前账户下所有承包商信息
     *
     * @return List<TCbsglxxEntity> 分页当前账户下所有承包商信息
     */
    @Operation(description = "分页查询承包商管理信息表数据", summary = "分页查询承包商管理信息表数据", tags = {"分页查询所有数据"})
    @GetMapping("page")
    @Parameters({
            @Parameter(description = "当前页", name = "current", in = ParameterIn.QUERY, required = true, schema = @Schema(implementation = Integer.class)),
            @Parameter(description = "当前页条数", name = "size", in = ParameterIn.QUERY, required = true, schema = @Schema(implementation = Integer.class))
    })
    @Idempotent(expireTime = 180, info = "3分钟内最多请求一次!", key = "#page.current")
    public IPage<TCbsglxxEntity> selectPage(@ParameterObject Page<TCbsglxxEntity> page, @ParameterObject TCbsglxxEntity tCbsglxxEntity) {
        return this.tCbsglxxService.page(page, Wrappers.query(tCbsglxxEntity));
    }
}
