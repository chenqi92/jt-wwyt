...
public class TQzpxdaController {
     ...
     * 查询当前账户下所有取证培训档案表     *
     *
     * @return List<TQzpxdaEntity> 当前账户下所有取证培训档案表
     */
    @Operation(description = "查询取证培训档案表信息表数据", summary = "查询取证培训档案表信息表数据", tags = {"查询所有数据"})
    @GetMapping
    @Idempotent(expireTime = 180, info = "3分钟内最多请求一次!")
    public List<TQzpxdaVO> selectAll() {
        return this.tQzpxdaService.selectList();
    }

    /**
     * 分页查询当前账户下所有取证培训档案表     *
     *
     * @return List<TQzpxdaDTO> 分页当前账户下所有取证培训档案表
     */
    ...
    public IPage<TQzpxdaVO> selectPage(@ParameterObject Page<TQzpxdaDTO> page, @ParameterObject TQzpxdaDTO tQzpxdaDTO) {
        return this.tQzpxdaService.selectPage(page, tQzpxdaDTO);
    }
