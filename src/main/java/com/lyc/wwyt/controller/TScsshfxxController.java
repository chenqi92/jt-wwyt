...
public class TScsshfxxController {
     ...
     * 查询当前账户下所有生产设备恢复信息     *
     *
     * @return List<TScsshfxxEntity> 当前账户下所有生产设备恢复信息
     */
    @Operation(description = "查询生产设备恢复信息信息表数据", summary = "查询生产设备恢复信息信息表数据", tags = {"查询所有数据"})
    @GetMapping
    @Idempotent(expireTime = 180, info = "3分钟内最多请求一次!")
    public List<TScsshfxxVO> selectAll() {
        return this.tScsshfxxService.selectList();
    }

    /**
     * 分页查询当前账户下所有生产设备恢复信息     *
     *
     * @return List<TScsshfxxDTO> 分页当前账户下所有生产设备恢复信息
     */
    ...
    public IPage<TScsshfxxVO> selectPage(@ParameterObject Page<TScsshfxxDTO> page, @ParameterObject TScsshfxxDTO tScsshfxxDTO) {
        return this.tScsshfxxService.selectPage(page, tScsshfxxDTO);
    }
