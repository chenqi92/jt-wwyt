...
public class TDlzyController {
    ...
    public void save(@RequestBody @Valid List<TDlzyDTO> list) {
        ...
        if (!CollectionUtils.isEmpty(entityList)) {
            for (TDlzyEntity dto : entityList) {
     ...
     * 查询当前账户下所有断路作业信息表信息
     *
     * @return List<TDlzyEntity> 当前账户下所有断路作业信息表信息
     */
    @Operation(description = "查询断路作业信息表信息表数据", summary = "查询断路作业信息表信息表数据", tags = {"查询所有数据"})
    @GetMapping
    @Idempotent(expireTime = 180, info = "3分钟内最多请求一次!")
    @ExportExcel(name = "断路作业信息表", sheets = @Sheet(sheetName = "t_dlzy"), headGenerator = CustomHead.class)
    public List<TDlzyVO> selectAll(@ParameterObject TDlzyDTO tDlzyDTO) {
        return this.tDlzyService.queryList(tDlzyDTO);
    }

    /**
     * 分页查询当前账户下所有断路作业信息表信息
     *
     * @return List<TDlzyDTO> 分页当前账户下所有断路作业信息表信息
     */
    ...
    public IPage<TDlzyVO> selectPage(@ParameterObject Page<TDlzyDTO> page, @ParameterObject TDlzyDTO tDlzyDTO) {
        return this.tDlzyService.queryPage(page, tDlzyDTO);
    }
    ...
    public List<TableInfoVO> heads() {
        // 查询所有列名
        List<TableInfoVO> queryInfos = this.commonService.queryTableHeaders("t_dlzy");
