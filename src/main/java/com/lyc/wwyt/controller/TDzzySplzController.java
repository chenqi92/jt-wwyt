...
public class TDzzySplzController {
    ...
    /**
     * 查询当前账户下所有吊装作业审批流转记录表     *
     *
     * @return List<TDzzySplzEntity> 当前账户下所有吊装作业审批流转记录表
     */
    @Operation(description = "查询吊装作业审批流转记录表信息表数据", summary = "查询吊装作业审批流转记录表信息表数据", tags = {"查询所有数据"})
    @GetMapping
    @Idempotent(expireTime = 180, info = "3分钟内最多请求一次!")
    public List<TDzzySplzVO> selectAll() {
        return this.tDzzySplzService.selectList();
    }

    /**
     * 分页查询当前账户下所有吊装作业审批流转记录表     *
     *
     * @return List<TDzzySplzDTO> 分页当前账户下所有吊装作业审批流转记录表
     */
    ...
    public IPage<TDzzySplzVO> selectPage(@ParameterObject Page<TDzzySplzDTO> page, @ParameterObject TDzzySplzDTO tDzzySplzDTO) {
        return this.tDzzySplzService.selectPage(page, tDzzySplzDTO);
    }
