...
public class TGczySplzController {
    ...
    /**
     * 查询当前账户下所有高处作业审批流转记录表     *
     *
     * @return List<TGczySplzEntity> 当前账户下所有高处作业审批流转记录表
     */
    @Operation(description = "查询高处作业审批流转记录表信息表数据", summary = "查询高处作业审批流转记录表信息表数据", tags = {"查询所有数据"})
    @GetMapping
    @Idempotent(expireTime = 180, info = "3分钟内最多请求一次!")
    public List<TGczySplzVO> selectAll() {
        return this.tGczySplzService.selectList();
    }

    /**
     * 分页查询当前账户下所有高处作业审批流转记录表     *
     *
     * @return List<TGczySplzDTO> 分页当前账户下所有高处作业审批流转记录表
     */
    ...
    public IPage<TGczySplzVO> selectPage(@ParameterObject Page<TGczySplzDTO> page, @ParameterObject TGczySplzDTO tGczySplzDTO) {
        return this.tGczySplzService.selectPage(page, tGczySplzDTO);
    }
