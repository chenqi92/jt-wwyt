...
public class TYhpcjlController {
     ...
     * 查询当前账户下所有隐患排查记录表     *
     *
     * @return List<TYhpcjlEntity> 当前账户下所有隐患排查记录表
     */
    @Operation(description = "查询隐患排查记录表信息表数据", summary = "查询隐患排查记录表信息表数据", tags = {"查询所有数据"})
    @GetMapping
    @Idempotent(expireTime = 180, info = "3分钟内最多请求一次!")
    public List<TYhpcjlVO> selectAll() {
        return this.tYhpcjlService.selectList();
    }

    /**
     * 分页查询当前账户下所有隐患排查记录表     *
     *
     * @return List<TYhpcjlDTO> 分页当前账户下所有隐患排查记录表
     */
    ...
    public IPage<TYhpcjlVO> selectPage(@ParameterObject Page<TYhpcjlDTO> page, @ParameterObject TYhpcjlDTO tYhpcjlDTO) {
        return this.tYhpcjlService.selectPage(page, tYhpcjlDTO);
    }
