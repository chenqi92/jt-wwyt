...
public class TCbssgglxxController {
     ...
     * 查询当前账户下所有承包商施工管理信息表     *
     *
     * @return List<TCbssgglxxEntity> 当前账户下所有承包商施工管理信息表
     */
    @Operation(description = "查询承包商施工管理信息表信息表数据", summary = "查询承包商施工管理信息表信息表数据", tags = {"查询所有数据"})
    @GetMapping
    @Idempotent(expireTime = 180, info = "3分钟内最多请求一次!")
    public List<TCbssgglxxVO> selectAll() {
        return this.tCbssgglxxService.selectList();
    }

    /**
     * 分页查询当前账户下所有承包商施工管理信息表     *
     *
     * @return List<TCbssgglxxDTO> 分页当前账户下所有承包商施工管理信息表
     */
    ...
    public IPage<TCbssgglxxVO> selectPage(@ParameterObject Page<TCbssgglxxDTO> page, @ParameterObject TCbssgglxxDTO tCbssgglxxDTO) {
        return this.tCbssgglxxService.selectPage(page, tCbssgglxxDTO);
    }
