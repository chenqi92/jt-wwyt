...
public class TQyjcxxController {
     ...
     * 查询当前账户下所有企业基础信息表     *
     *
     * @return List<TQyjcxxEntity> 当前账户下所有企业基础信息表
     */
    @Operation(description = "查询企业基础信息表信息表数据", summary = "查询企业基础信息表信息表数据", tags = {"查询所有数据"})
    @GetMapping
    @Idempotent(expireTime = 180, info = "3分钟内最多请求一次!")
    public List<TQyjcxxVO> selectAll() {
        return this.tQyjcxxService.selectList();
    }

    /**
     * 分页查询当前账户下所有企业基础信息表     *
     *
     * @return List<TQyjcxxDTO> 分页当前账户下所有企业基础信息表
     */
    ...
    public IPage<TQyjcxxVO> selectPage(@ParameterObject Page<TQyjcxxDTO> page, @ParameterObject TQyjcxxDTO tQyjcxxDTO) {
        return this.tQyjcxxService.selectPage(page, tQyjcxxDTO);
    }
