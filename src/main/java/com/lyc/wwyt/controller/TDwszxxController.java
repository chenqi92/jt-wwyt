...
public class TDwszxxController {
     ...
     * 查询当前账户下所有点位设置信息表信息
     *
     * @return List<TDwszxxEntity> 当前账户下所有点位设置信息表信息
     */
    @Operation(description = "查询点位设置信息表信息表数据", summary = "查询点位设置信息表信息表数据", tags = {"查询所有数据"})
    @GetMapping
    @Idempotent(expireTime = 180, info = "3分钟内最多请求一次!")
    @ExportExcel(name = "点位设置信息表", sheets = @Sheet(sheetName = "t_dwszxx"), headGenerator = CustomHead.class)
    public List<TDwszxxVO> selectAll(@ParameterObject TDwszxxDTO tDwszxxDTO) {
        return this.tDwszxxService.queryList(tDwszxxDTO);
    }

    /**
     * 分页查询当前账户下所有点位设置信息表信息
     *
     * @return List<TDwszxxDTO> 分页当前账户下所有点位设置信息表信息
     */
    ...
    public IPage<TDwszxxVO> selectPage(@ParameterObject Page<TDwszxxDTO> page, @ParameterObject TDwszxxDTO tDwszxxDTO) {
        return this.tDwszxxService.queryPage(page, tDwszxxDTO);
    }
    ...
    public List<TableInfoVO> heads() {
        // 查询所有列名
        List<TableInfoVO> queryInfos = this.commonService.queryTableHeaders("t_dwszxx");
