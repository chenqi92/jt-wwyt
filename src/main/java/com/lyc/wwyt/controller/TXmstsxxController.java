...
public class TXmstsxxController {
     ...
     * 查询当前账户下所有项目三同时信息表信息
     *
     * @return List<TXmstsxxEntity> 当前账户下所有项目三同时信息表信息
     */
    @Operation(description = "查询项目三同时信息表信息表数据", summary = "查询项目三同时信息表信息表数据", tags = {"查询所有数据"})
    @GetMapping
    @Idempotent(expireTime = 180, info = "3分钟内最多请求一次!")
    @ExportExcel(name = "项目三同时信息表", sheets = @Sheet(sheetName = "t_xmstsxx"), headGenerator = CustomHead.class)
    public List<TXmstsxxVO> selectAll(@ParameterObject TXmstsxxDTO tXmstsxxDTO) {
        return this.tXmstsxxService.queryList(tXmstsxxDTO);
    }

    /**
     * 分页查询当前账户下所有项目三同时信息表信息
     *
     * @return List<TXmstsxxDTO> 分页当前账户下所有项目三同时信息表信息
     */
    ...
    public IPage<TXmstsxxVO> selectPage(@ParameterObject Page<TXmstsxxDTO> page, @ParameterObject TXmstsxxDTO tXmstsxxDTO) {
        return this.tXmstsxxService.queryPage(page, tXmstsxxDTO);
    }
    ...
    public List<TableInfoVO> heads() {
        // 查询所有列名
        List<TableInfoVO> queryInfos = this.commonService.queryTableHeaders("t_xmstsxx");
