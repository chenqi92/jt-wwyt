...
public class TPxhdxxController {
     ...
     * 查询当前账户下所有培训活动信息表     *
     *
     * @return List<TPxhdxxEntity> 当前账户下所有培训活动信息表
     */
    @Operation(description = "查询培训活动信息表信息表数据", summary = "查询培训活动信息表信息表数据", tags = {"查询所有数据"})
    @GetMapping
    @Idempotent(expireTime = 180, info = "3分钟内最多请求一次!")
    public List<TPxhdxxVO> selectAll() {
        return this.tPxhdxxService.selectList();
    }

    /**
     * 分页查询当前账户下所有培训活动信息表     *
     *
     * @return List<TPxhdxxDTO> 分页当前账户下所有培训活动信息表
     */
    ...
    public IPage<TPxhdxxVO> selectPage(@ParameterObject Page<TPxhdxxDTO> page, @ParameterObject TPxhdxxDTO tPxhdxxDTO) {
        return this.tPxhdxxService.selectPage(page, tPxhdxxDTO);
    }
