...
public class TDwjzglxxController {
    ...
    /**
     * 查询当前账户下所有定位基站管理信息表信息
     *
     * @return List<TDwjzglxxEntity> 当前账户下所有定位基站管理信息表信息
     */
    @Operation(description = "查询定位基站管理信息表信息表数据", summary = "查询定位基站管理信息表信息表数据", tags = {"查询所有数据"})
    @GetMapping
    @Idempotent(expireTime = 180, info = "3分钟内最多请求一次!")
    @ExportExcel(name = "定位基站管理信息表", sheets = @Sheet(sheetName = "t_dwjzglxx"), headGenerator = CustomHead.class)
    public List<TDwjzglxxVO> selectAll(@ParameterObject TDwjzglxxDTO tDwjzglxxDTO) {
        return this.tDwjzglxxService.queryList(tDwjzglxxDTO);
    }

    /**
     * 分页查询当前账户下所有定位基站管理信息表信息
     *
     * @return List<TDwjzglxxDTO> 分页当前账户下所有定位基站管理信息表信息
     */
    ...
    public IPage<TDwjzglxxVO> selectPage(@ParameterObject Page<TDwjzglxxDTO> page, @ParameterObject TDwjzglxxDTO tDwjzglxxDTO) {
        return this.tDwjzglxxService.queryPage(page, tDwjzglxxDTO);
    }
    ...
    public List<TableInfoVO> heads() {
        // 查询所有列名
        List<TableInfoVO> queryInfos = this.commonService.queryTableHeaders("t_dwjzglxx");
