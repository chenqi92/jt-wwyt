...
public class TScqyxxController {
     ...
     * 查询当前账户下所有生产区域信息表信息
     *
     * @return List<TScqyxxEntity> 当前账户下所有生产区域信息表信息
     */
    @Operation(description = "查询生产区域信息表信息表数据", summary = "查询生产区域信息表信息表数据", tags = {"查询所有数据"})
    @GetMapping
    @Idempotent(expireTime = 180, info = "3分钟内最多请求一次!")
    @ExportExcel(name = "生产区域信息表", sheets = @Sheet(sheetName = "t_scqyxx"), headGenerator = CustomHead.class)
    public List<TScqyxxVO> selectAll(@ParameterObject TScqyxxDTO tScqyxxDTO) {
        return this.tScqyxxService.queryList(tScqyxxDTO);
    }

    /**
     * 分页查询当前账户下所有生产区域信息表信息
     *
     * @return List<TScqyxxDTO> 分页当前账户下所有生产区域信息表信息
     */
    ...
    public IPage<TScqyxxVO> selectPage(@ParameterObject Page<TScqyxxDTO> page, @ParameterObject TScqyxxDTO tScqyxxDTO) {
        return this.tScqyxxService.queryPage(page, tScqyxxDTO);
    }
    ...
    public List<TableInfoVO> heads() {
        // 查询所有列名
        List<TableInfoVO> queryInfos = this.commonService.queryTableHeaders("t_scqyxx");
