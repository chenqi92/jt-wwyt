...
public class TGczyAqcsController {
    ...
    /**
     * 查询当前账户下所有高处作业信息安全措施表信息
     *
     * @return List<TGczyAqcsEntity> 当前账户下所有高处作业信息安全措施表信息
     */
    @Operation(description = "查询高处作业信息安全措施表信息表数据", summary = "查询高处作业信息安全措施表信息表数据", tags = {"查询所有数据"})
    @GetMapping
    @Idempotent(expireTime = 180, info = "3分钟内最多请求一次!")
    @ExportExcel(name = "高处作业信息安全措施表", sheets = @Sheet(sheetName = "t_gczy_aqcs"), headGenerator = CustomHead.class)
    public List<TGczyAqcsVO> selectAll(@ParameterObject TGczyAqcsDTO tGczyAqcsDTO) {
        return this.tGczyAqcsService.queryList(tGczyAqcsDTO);
    }

    /**
     * 分页查询当前账户下所有高处作业信息安全措施表信息
     *
     * @return List<TGczyAqcsDTO> 分页当前账户下所有高处作业信息安全措施表信息
     */
    ...
    public IPage<TGczyAqcsVO> selectPage(@ParameterObject Page<TGczyAqcsDTO> page, @ParameterObject TGczyAqcsDTO tGczyAqcsDTO) {
        return this.tGczyAqcsService.queryPage(page, tGczyAqcsDTO);
    }

    /**
     * 查询高处作业信息安全措施表的所有字段信息
     *
     * @return 所有字段信息
     */
    @GetMapping("heads")
    ...
    public List<TableInfoVO> heads() {
        // 查询所有列名
        List<TableInfoVO> queryInfos = this.commonService.queryTableHeaders("t_gczy_aqcs");
