...
public class TZyqyxxController {
     ...
     * 查询当前账户下所有作业区域信息表信息
     *
     * @return List<TZyqyxxEntity> 当前账户下所有作业区域信息表信息
     */
    @Operation(description = "查询作业区域信息表信息表数据", summary = "查询作业区域信息表信息表数据", tags = {"查询所有数据"})
    @GetMapping
    @Idempotent(expireTime = 180, info = "3分钟内最多请求一次!")
    @ExportExcel(name = "作业区域信息表", sheets = @Sheet(sheetName = "t_zyqyxx"), headGenerator = CustomHead.class)
    public List<TZyqyxxVO> selectAll(@ParameterObject TZyqyxxDTO tZyqyxxDTO) {
        return this.tZyqyxxService.queryList(tZyqyxxDTO);
    }

    /**
     * 分页查询当前账户下所有作业区域信息表信息
     *
     * @return List<TZyqyxxDTO> 分页当前账户下所有作业区域信息表信息
     */
    ...
    public IPage<TZyqyxxVO> selectPage(@ParameterObject Page<TZyqyxxDTO> page, @ParameterObject TZyqyxxDTO tZyqyxxDTO) {
        return this.tZyqyxxService.queryPage(page, tZyqyxxDTO);
    }
    ...
    public List<TableInfoVO> heads() {
        // 查询所有列名
        List<TableInfoVO> queryInfos = this.commonService.queryTableHeaders("t_zyqyxx");
