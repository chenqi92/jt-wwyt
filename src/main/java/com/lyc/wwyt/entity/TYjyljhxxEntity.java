...
public class TYjyljhxxEntity extends BaseEntity {
    ...
    @NotBlank(message = "演练方式：1-综合；2-桌面；3-专项不能为空!")
    @Size(max = 10, message = "演练方式：1-综合；2-桌面；3-专项不能超过10个字符(1个汉字记两个字符)!")
    private String ylfs;

    @Schema(description = "应急预案主键UUID", name = "yjya", implementation = String.class, maxLength = 36)
    @NotBlank(message = "应急预案主键UUID不能为空!")
    @Size(max = 36, message = "应急预案主键UUID不能超过36个字符(1个汉字记两个字符)!")
    private String yjya;

    @Schema(description = "演练级别：1-公司级；2-分厂级；3-车间级", name = "yljb", implementation = String.class, maxLength = 10)
    @NotBlank(message = "演练级别：1-公司级；2-分厂级；3-车间级不能为空!")
    @Size(max = 10, message = "演练级别：1-公司级；2-分厂级；3-车间级不能超过10个字符(1个汉字记两个字符)!")
    private String yljb;
